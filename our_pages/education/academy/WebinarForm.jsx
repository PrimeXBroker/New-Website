...
function WebinarForm() {
    ...
    const fetchUpcomingWebinars = async () => {
        const res = await axios.get("https://primexbroker.com/api/upcoming-webinars")
        if (res.data.success) {
            setUpcoming(
    ...
    const formik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                // setLoading(true);
                Object.assign(values, { language: locale, country: originCountry, source: "website academy" })

                const res = await axios.post("https://primexbroker.com/api/zoom/register/participant", values)
                if (res.data.success) {
                    formik.resetForm();
                    setLoading(false)
                    onOpen()
            ...
            <div className="shadow-xl bg-[#fff] border-accent border xl:w-[40%] xl:mx-auto lg:w-[40%] lg:mx-auto md:w-[50%] md:mx-auto sm:w-[90%] sm:mx-auto w-[90%] mx-auto rounded-2xl">
                ...
                >
                    ...
                    <div className="mb-10 w-[80%]">
                        <select
                            className={`bg-white text-gray-400 w-full placeholder:text-gray-300 outline-none border-b border-b- capitalize pt-[12px] pb-[0.5rem] px-4 rounded-[5px] ${formik.touched.webinarId && formik.errors.webinarId
                                ? "border-b border-red-600"
                                : ""
                                }`}
                            name="webinarId"
                    ...
                    <button className="bg-primary shadow-xl rounded-full font-semibold px-3 py-2 text-secondary w-[150px] absolute bottom-[-30px] mx-auto">
                        <div className="flex gap-3 items-center text-center px-8">
                          {loading && (
                ...
                <ModalContent>
                    {(onClose) => (
                        <>
                            ...
                            <ModalBody>
                                <p>
                                    Thanks For Subscribing to PrimeX Capital Academy
                                </p>
