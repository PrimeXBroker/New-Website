...
const MarketNews = () => {
  ...
  const fetchNews = async () => {
    setLoading(true)
    const res = locale === "ar" ? await axios.get(`https://primexbroker.com/api/fetch/published/blogs/${page}/9/news/ar`) : await axios.get(`https://primexbroker.com/api/fetch/published/blogs/${page}/9/all/${locale}`)
    if (res.data.success) {
  ...
  return (
    <section className="container py-20">
      <div className="grid grid-cols-12 ">
        {news.map((blog, index) => (
          <div  className="lg:col-span-4 md:col-span-6  col-span-12 px-4 mb-4">
            <Link href={`/${locale}/market-news/${blog.slug}`} className="group">
      ...
      <div className="flex justify-center mt-10">
        <Pagination showControls total={totalPages} initialPage={page} onChange={(p)=> setPage(p) } />
      </div>
